...
function Invoke-CreateCertificate([string] $certSubject, [bool] $isCA)
{
	...
	else
	{
		...
		$Cert = New-SelfSignedCertificate -Signer $CA -Type Custom -Subject "CN=$certSubject"  -KeyUsage @("CertSign", "KeyAgreement", "DataEncipherment", "KeyEncipherment", "NonRepudiation", "DigitalSignature") -KeyAlgorithm ECDSA_nistP256 -CurveExport CurveName -CertStoreLocation "Cert:\CurrentUser\My"
	}
    ...
    While($CnQueue.count -ne 0) {
        ...
        $process = {
			...
			function Invoke-CreateCertificate([string] $certSubject, [bool] $isCA)
			{
				...
				$issuer = "__Interceptorv4_ROOTCA_DO_NOT_TRUST"
				...
				{              
					$signer = (Get-ChildItem Cert:\CurrentUser\Root | Where-Object {$_.Subject -match "__Interceptorv4_ROOTCA_DO_NOT_TRUST" })
				...
				{              
					...
					$CACertificate = (Get-ChildItem Cert:\CurrentUser\My | Where-Object {$_.Subject -match "__Interceptorv4_ROOTCA_DO_NOT_TRUST" })
