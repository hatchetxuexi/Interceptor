...
function Invoke-CreateRootCertificate([string] $certSubject)
{
	
	# Create and Install the CA Certificate.
	#$CA= New-SelfSignedCertificate -Type Custom -Subject "CN=$certSubject" -KeyUsage CertSign -KeyAlgorithm ECDSA_nistP256 -CurveExport CurveName -CertStoreLocation "Cert:\CurrentUser\My"
	
	$CA= New-SelfSignedCertificate -Type Custom -Subject "CN=$certSubject" -KeyUsage CertSign -KeyAlgorithm RSA -KeyLength 2048 -CertStoreLocation "Cert:\CurrentUser\My"
...
Register-ObjectEvent -SourceIdentifier MonitorClientConnection -InputObject $Timer -EventName Elapsed -Action {
    While($CnQueue.count -ne 0) {
        ...
        $process = {
			...
			function Invoke-CreateLeafCertificate([string] $certSubject)
			{
				#[TODO]Fix This to be parameter, looking up each create is slow...
				$CA = (Get-ChildItem "Cert:\CurrentUser\My" | Where-Object {$_.Subject -match "__Interceptorv4_ROOTCA_DO_NOT_TRUST" })
				
				#$Cert = New-SelfSignedCertificate -Type Custom -Signer $CA -Type Custom -Subject "CN=$certSubject" -DnsName "$certSubject" -KeyUsage @("CertSign", "KeyAgreement", "DataEncipherment", "KeyEncipherment", "NonRepudiation", "DigitalSignature") -KeyAlgorithm ECDSA_nistP256 -CurveExport CurveName -CertStoreLocation "Cert:\CurrentUser\My"
				$Cert = New-SelfSignedCertificate -Type Custom -Signer $CA -DnsName "$certSubject" -KeyUsage @("CertSign", "KeyAgreement", "DataEncipherment", "KeyEncipherment", "NonRepudiation", "DigitalSignature") -KeyAlgorithm RSA -KeyLength 2048 -CertStoreLocation "Cert:\CurrentUser\My"
			...
			function Receive-ClientHttpRequest([System.Net.Sockets.TcpClient] $client, [System.Net.WebProxy] $proxy)
			{
				...
				{	
					...
					if($methodParse[0] -ceq "CONNECT")
					{
						...
						{
							#Debug Time 
							[System.Console]::WriteLine("$(Get-Date)")
							$sslcertfake = Invoke-CreateLeafCertificate $domainParse[0] 
						}
						else
						{
							$sslcertfake = $sslcertfake[0] #{TODO] Correct duplicate cert issues
						}
...
$CACertificateCheck = (Get-ChildItem Cert:\CurrentUser\My | Where-Object {$_.Subject -match "__Interceptorv4_ROOTCA_DO_NOT_TRUST" }) 
						
							
if ($CACertificateCheck -eq $null)
{
[console]::WriteLine("**** CA Cert Created *******`n")
Invoke-CreateRootCertificate "__Interceptorv4_ROOTCA_DO_NOT_TRUST"
}
else
{
...
unction Start-CertificateAuthority()
{
	...
	Start-Job -ScriptBlock {
			...
			While ($Hso.IsListening) {
				...
				$cert = Get-ChildItem "cert:\CurrentUser\Root" | where { $_.Issuer -match "__Interceptorv4_ROOTCA_DO_NOT_TRUST" }
