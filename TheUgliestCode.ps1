function Invoke-CreateCertificate([string] $certSubject, [bool] $isCA)
{
	if($isCA)
	{
		...
		$CA= New-SelfSignedCertificate -Type Custom -Subject "CN=$certSubject" -KeyUsageProperty @("All") -KeyUsage CertSign -KeyAlgorithm ECDSA_nistP256 -CurveExport CurveName -CertStoreLocation "Cert:\CurrentUser\My"
	...
	else
	{
		...
		$Cert = New-SelfSignedCertificate -Signer $CA -Type Custom -Subject "CN=$certSubject" -KeyUsageProperty @("All")   -KeyUsage @("CertSign", "KeyAgreement", "DataEncipherment", "KeyEncipherment", "NonRepudiation", "DigitalSignature") -DnsName "$certSubject"  -KeyAlgorithm ECDSA_nistP256 -CurveExport CurveName -CertStoreLocation "Cert:\CurrentUser\My"
	}
